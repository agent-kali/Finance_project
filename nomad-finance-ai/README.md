...
> **Not wired in:** there is no FX API (e.g. Frankfurter) — `EXCHANGE_RATES` in `lib/constants.ts` are static. There is also no CSV import path; transactions are created via the `add-transaction-sheet.tsx` form (calling `createTransaction` in `app/actions/transactions.ts`) and demo data is generated by `seedDemoData()` in `app/actions/demo-seed.ts`.
